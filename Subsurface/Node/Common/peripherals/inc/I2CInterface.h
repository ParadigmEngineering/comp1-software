...
public:
	...
	// Returns a pointer to the data received buffer
	...
	// Getter and setter for I2C addressing mode (10 bit or 8 bit)
	...
	// Master transmit and receive functions (polling mode)
	HAL_StatusTypeDef masterTransmit(uint16_t address,
	                                 uint8_t *data,
	                                 uint32_t timeout,
	                                 uint8_t size);

	HAL_StatusTypeDef masterReceive(uint16_t address,
	                                uint32_t timeout);

	// Reading / writing from device internal memory addresses
	HAL_StatusTypeDef memWrite(uint8_t *data,
	                           uint16_t devAddress,
	                           uint16_t memAddress,
	...
	HAL_StatusTypeDef memRead(uint16_t devAddress,
	                          uint16_t memAddress,
	                          uint16_t memAddressSize,
	                          uint32_t timeout);

	HAL_StatusTypeDef isDeviceReady(uint16_t devAddress,
	                                uint32_t timeout,
	                                uint32_t trials = 100);
